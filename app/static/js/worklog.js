...
$(document).ready(function() {
    ...
    $("#issues-wrap").on("click", "#issues-tb :button", function() {
        ...
        if (!exist) {
            ...
            reportObj.appendTo("#report-tb").data({
                key: objId,
                action: "create"
            });
    ...
    $("#add-other span.btn").on("click", function() {
        ...
        if ($.trim(keyword) === "") {
            ...
            timeout_exec(function() {
                $inputTxt.removeClass("has-error");
            }, 1000);
            return;
        }
...
function orderInit() {
    var init = function() {
        $("#reportForm textarea").textareaAutoResize({
            rows: 1,
            cols: 75,
            maxRow: 10
        });
        ...
        worklog.global.needWelcome() &&
            $("#collapse").popover({
                "content": "Hey, Welcome DDRT. Click here to add more issue !"
            }).popover('show');
...
var showIssues = function(data, keepOld) {
    ...
    $.each(data.issues, function(k, v) {
        issueWrap.append(('<tr><th scope="row">' + (k + 1) + '</th><td><img src="' + v.fields.issuetype.iconUrl + '" height="16" width="16" border="0" align="absmiddle" title="' + v.fields.issuetype.description + '">' + v.fields.issuetype.name + '</td><td><a href="' + getBrowseUrl(v.key) + '" target="_blank">' + v.key + '</a></td><td>' + (v.fields.summary || "N/A") + '</td><td><button type="button" name=' + v.key + ' class="btn btn-default btn-xs font-bold" title="add a report"> + </button>' + (isSelected(v.key) ? worklog.global.checkedTag : '') + '</td></tr>'));
    });
};
...
var initPagination = function(query) {
    ...
    return sendAjax("api/v1/jira/search",
        function() {
            showLoading("Loding issue, please waiting...");
        },
        function(data) {
            showIssues(data);
        ...
        timeout_exec(closeLoading, 1000);
    });
};
...
var searchIssues = function(query, setResult) {
    return sendAjax("api/v1/jira/search", function() {
        showLoading("Loding issue, please waiting...")
    }, function(data) {
        setResult(data);
...
var initInterface = function(data, jira_data) {
    ...
    for (var i = 0; i < len; i++) {
        objId = data[i].issue;
        reportObj = worklog.getReportObj(objId);
        reportObj.find("th").html('<p class="issue-summary"><a href="' + getBrowseUrl(objId) + '" target="_blank">' + objId + '</a><i style="display: block;">' + formatSummary(findIssue(jira_data.issues, objId).fields.summary || "") + '</i></p>');
...
var initInterfaceHistory = function(jira_data) {
    var len = jira_data.issues.length,
        reportObj = false;
    $.each(jira_data.issues, function() {
        reportObj = worklog.getReportObj(this.key);
        reportObj.find("th").html('<p class="issue-summary"><a href="' + getBrowseUrl(this.key) + '" target="_blank">' + this.key + '</a><i style="display: block;">' + (this.fields.summary || "") + '</i></p>');
        reportObj.appendTo("#report-tb").data({
            key: this.key,
            action: "create"
        });
